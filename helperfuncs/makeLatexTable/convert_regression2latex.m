function latexCode = convert_regression2latex(mdl,modelNames,info,round_digits,label,orientation)
%   Create a latex table code containing the output of the linear model (created by fitlm, fitglm).

%    latexCode = convert_regression2latex(mdl,modelNames,info,round_digits,orientation)

%   'latex_code'    - table latex code
%   'mdl'           - output of fitlm/fitglm. If more than one model are to
...
results = prepare_fitlm4latex(mdl,info);
...
if nEmptyCells==0 % means that all columns use the same model
...
latexCode=['%   Table generated by the matlab function: convert_regression2latex.m';
    '%   Written by Pierre Petitet (pierre.petitet@gmail.com)';...
...
function results=prepare_fitlm4latex(mdl,info)
    ...
    switch info{i}
        case 'tStat'
            result.output.tStat = horzcat(mdl.Coefficients.tStat,... % t-stat
                repmat(mdl.DFE,[length(mdl.Coefficients.tStat) 1])); % DF
        case 'F'
            result.output.F = horzcat(mdl.anova.DF,... % DF1
                mdl.anova.F); % F-stat
        case 'R2'
            result.extra.adjR2=mdl.Rsquared.Ordinary; % ordinary R2
        case 'adjR2'
            result.extra.adjR2=mdl.Rsquared.Adjusted; % ajudsted R2
        case 'n'
            result.extra.n=mdl.NumObservations; % number of observation
        case {'AIC', 'BIC', 'AICc', 'CAIC'}
