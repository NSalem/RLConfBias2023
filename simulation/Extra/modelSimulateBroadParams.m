...
load('Results\RLvars_all.mat', 'params_allexp','modelsinfo')
% load confidence parameters
reg = load('Results\reg_conflogit_learning_dqabs.mat');
regPost = load('Results\reg_conflogit_posttest_dqabs.mat');
learnmodel = 11;
...
%         p = gamfit(learnparamssub(:,iparam));
        learnparams(:,iparam) =random('Gamma',1.2,5,size(learnparams,1),1);
    else
%         p = betafit(learnparamssub(:,iparam));
        learnparams(:,iparam) = random('Beta',1.1,1.1,size(learnparams,1),1);
    end
end
...
for iconfmodelLT = 1:numel(idcLT)
    ...
    idc = all(abs(zscore(coeffsThisModel))<=2.5,2);
    pmax = max(coeffsThisModel(idc,:));
    pmin = min(coeffsThisModel(idc,:));   
    
    confparamsLT{iconfmodelLT} =  rand(nsub,size(reg.confcoeff{idcLT(iconfmodelLT)},2)).*(pmax-pmin)+pmin;
...
for iconfmodelTT = 1:numel(idcTT)
    ...
    pmax = max(coeffsThisModel(idc,:));
    pmin = min(coeffsThisModel(idc,:));       
    confparamsTT{iconfmodelTT} =  rand(nsub,size(regPost.confcoeff{idcTT(iconfmodelTT)},2)).*(pmax-pmin)+pmin;
...
for isim = 1:nsims
    ...
    %simulate RL
    [outRL,outSorted]= simulateRLPop(learncell,modelsinfo(11),3, 24, outcomes);
...
save('Results\SimulationsSubjParamsUnif.mat','outRL','outSorted',...
