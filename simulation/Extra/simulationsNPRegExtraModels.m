...
for ilearnmodel =1:3
    load(['Results\SimulationsSubjParams',rlnames{ilearnmodel},'.mat'])
    
    ss_all = squeeze(outRL.ss);
    ss_all = permute(ss_all,[1,3,2]);
    aa_all = squeeze(outRL.choicePost);
    % contPost_all = outConf
    % [prefMat_sim,accMat_sim,confChoiceMat_sim,confChoicePairMat_sim]= makePostMats(ss_all,aa_all,confPost_all{iconfmodelTT});
    ...
    for imodel = 1:size(outConfLSorted,3)
        ...
        %     [prefMat_sim,accMat_sim,confChoiceMat_sim,confChoicePairMat_sim]= makePostMats(ss_all,aa_all,squeeze(outConfT(:,imodel,:)));
        %     prefMat(isub,:,:) = nanmean(prefMat_sim);
        %     accMat(isub,:,:,:) =nanmean(accMat_sim);
        %     confChoiceMat(iconfmodelTT,isub,:,:) = nanmean(confChoiceMat_sim);
        %     confChoicePairMat(iconfmodelTT,isub,:,:) = nanmean(confChoicePairMat_sim);
        
        %     prefMat = squeeze(nanmean(prefMat,1));
        %     accMat =
        
        %     for iconfmodelTT = 1:numel(idcTT)
        %             conf_matTT{iconfmodelTT}(isub,:) = conf_matTT_sim{iconfmodelTT}(isim,:);
        %     end
    end
    ...
    %%%%%%% TO DO: get mats of accuracy and confidence size subj x symbol
    %%%%%%% present
    
    % accPost = squeeze(nanmean(nanmean(accMat/4,4),1));
    % confPost = squeeze(nanmean(nanmean(confChoicePairMat
    ...
    for imodel = 1:4
        % squeeze(mean(conf_matTT{1}))
        %     meanConfCond = squeeze(confPost(imodel,:,:));
    ...
    learnparamnames = {'$\beta$','$\alpha_{CON}$','$\alpha_{DIS}$','$\alpha_V$','w','$\phi$','$\tau$','$\frac{\alpha_{CON}-\alpha_{DIS}}{\alpha_{CON}+\alpha_{DIS}}$','$\alpha_{CON}-\alpha_{DIS}$'};
            ...
            for ipartype = 1:2
                ...
                for ipar = 1:size(params,2)
                    ...
                    for imeas = 1:4
                        ...
                        %                     r2Lin = thislm.Rsquared.Ordinary;
    ...
    save(['Results\NonParRegSims',rlnames{ilearnmodel},'.mat'],'regAllX','regAllF','regAllCI95','rho', 'p','regLin');
end
